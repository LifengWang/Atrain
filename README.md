...
* **Configure Smart Server**
   ...
   The active SSM server is the first node in the `servers` file under /conf. After failover, please using the following command to find new active SSM servers

   `hdfs cat /system/mover.id`
...
----------------------------------------------------------------------------------
We need some Hadoop configuration for integrating SSM to Hadoop.
...
FAQ
---------------------------------------------------
need to set proxy for compiling ssm
