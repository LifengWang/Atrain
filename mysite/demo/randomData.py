...
def randomData():
    ...
    for idx in xrange(0, count):
        f = Finance(_t + idx)
        f.randData()
        sql = '''insert into demotest (from_currency, to_currency, ratio, t_when)
values ('%s','%s',%f,%d) ''' % (f.from_currency, f.to_currency, f.ratio, f.t_when)
        cu.execute(sql)
        loop += 1
        if loop == 100:
            loop = 0
...
def createDB():
    cu = cx.cursor()
    try:
        cu.execute('''CREATE TABLE demotest (id  INTEGER PRIMARY KEY AUTOINCREMENT,
from_currency VARCHAR(10), to_currency VARCHAR(10), ratio DOUBLE, t_when INTEGER)''')
        cx.commit()
    except Exception, e:
        print e
    cu.close()
...
def dropDB():
    cu = cx.cursor()
    try:
        cu.execute('''DROP TABLE IF EXISTS demotest''')
    except Exception, e:
        print e
    cx.commit()
...
def getOneData(t_when):
    cu = cx.cursor()
    sql = '''select * from demotest where t_when=%s ''' % t_when
    rs = cu.execute(sql)
    for r in rs:
        return r
    cu.close()
...
def getDatas(start_when, end_when):
    result = []
    cu = cx.cursor()
    sql = '''select * from demotest where t_when>=%d and t_when<=%d''' % (start_when, end_when)
    rs = cu.execute(sql)
    for r in rs:
        result.append(r)
    cu.close()
...
def getResult():
    ...
    for idx in xrange(acc, 10000):
        ...
        if EMA5 > EMA13 and EMA5 > EMA20 and BUY_ON_FLAG == 0:
            BUY_ON_FLAG = 1
